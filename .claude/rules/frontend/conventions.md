...
`components/form/` holds the audience-agnostic wrappers that screen uses — `ComboboxField` (a listbox over Radix Popover: a floating layer anchored to the field's width and portalled to `<body>`, so no ancestor's `overflow` can clip it and opening it never changes the height of anything. Collision detection flips it above the field when the viewport — or the on-screen keyboard, which Floating UI sees through `visualViewport` — leaves no room below. It renders `role="presentation"` on the popover content so the `<ul role="listbox">` is exposed cleanly, and prevents Radix's open/close auto-focus so the focus stays on the input, which is what `aria-activedescendant` requires. It was an in-flow listbox until it was shown pushing the field below it off the page), `PhoneField` and `FieldMessage`, which owns the one rule every field shares: a server `error` supersedes a `hint`. `lib/http.ts` is the only module that knows Laravel's `{ message, errors }` envelope, so a 422 through axios renders exactly like a 422 through a Fortify post.
