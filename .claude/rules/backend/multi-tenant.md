...
Enforcement on a route is `App\Http\Middleware\RequirePermission`, aliased `permission` in `bootstrap/app.php` and applied per route — `->middleware('permission:view_services')`. It reads the caller's uuid, asks `BusinessAuthorization::grants()` against the current business, and throws `App\Http\Exceptions\PermissionDenied` on both failure paths: no authenticated caller and permission not held look identical from outside, and the message never names the permission. It throws rather than calling `abort()` because a `DomainFailure` carries its own classification — `RenderDomainFailure` turns it into the 403.

Planned and deliberately absent: the **platform plane** (roles assigned with a NULL team, for operating the system itself). The schema is ready for it — `scope` already distinguishes `platform` from `business` — but nothing is seeded and no endpoint uses it. Landing it needs one migration: `model_has_roles.business_id` is `NOT NULL` and part of the composite primary key, and Postgres here is 14, so `UNIQUE NULLS NOT DISTINCT` is unavailable and the replacement is an expression index over `coalesce(business_id, 0)`. What it needs is a team-less check, not a second middleware: `RequirePermission` resolves the business from `BusinessContext`, which a platform route would not bind.
