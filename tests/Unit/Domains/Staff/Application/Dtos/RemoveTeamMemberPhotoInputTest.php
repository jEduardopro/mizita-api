...
it('returns silently for a member uuid, in either case', function (string $staffMemberId) {
    expect(fn () => (new RemoveTeamMemberPhotoInput($staffMemberId))->validate())->not->toThrow(Throwable::class);
})->with([
...
it('refuses a member that is not a uuid as a member that is not there', function (string $staffMemberId) {
    expect(fn () => (new RemoveTeamMemberPhotoInput($staffMemberId))->validate())->toThrow(StaffMemberNotFound::class);
})->with([
    'empty' => '',
    'an int id' => '42',
    'garbage' => 'not-a-uuid',
