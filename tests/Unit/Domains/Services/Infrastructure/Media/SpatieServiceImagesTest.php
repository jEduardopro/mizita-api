...
describe('keeping Illuminate\Http out of the application layer', function () {
    it('takes the upload as a path and a name, both plain strings', function () {
        expect(array_map(
            ...
            serviceImagesMethod('replace')->getParameters(),
        ))->toBe(['serviceId:string', 'sourcePath:string', 'fileName:string']);
    });
    ...
    it('identifies the service it stores against by uuid, never an internal key', function (string $method) {
        $first = serviceImagesMethod($method)->getParameters()[0];

        expect($first->getName())->toBe('serviceId')
