...
/*
| A pure domain service: the candidates are handed to it, so it is instantiated
| for real - there is nothing to double, and doubling the thing under test would
| assert only that the test can write a list.
*/
...
it('starts numbering at two when the base is taken', function () {
    // The unsuffixed business is conceptually the first, so the second one is
    // "-2" rather than "-1".
...
it('fills a gap in the sequence rather than counting past it', function () {
    // A business gave up "-2" and the row went with it. The lowest free number
    // is the one to hand out; skipping to the end would number by history
    // instead of by what is free.
...
it('hands out the bare base when only numbered variants are taken', function () {
    // The base itself is free - whoever held it is gone - so nothing needs a
    // number at all.
...
it('ignores a slug that merely starts with the base', function () {
    // The case that motivated the design: "barberia-lopez-madrid" is a
    // different name that happens to begin the same way, and letting it consume
    // suffix 2 would make the numbering lie about how many businesses share a
    // name.
...
it('is not fooled by regex metacharacters in the base', function () {
    // The base is quoted before it is matched. A slug cannot contain a dot
    // today, but the allocator must not be the thing standing between a future
    // alphabet change and a wrong answer.
...
it('treats a suffix written with a leading zero as the number it spells', function () {
    // "-02" is not a slug this platform writes, but if one is in the table it
    // occupies two, and handing two out again would collide.
...
it('returns the very base it was given when the base is free', function () {
    // Identity, not just an equal value: nothing is rebuilt on the free path.
