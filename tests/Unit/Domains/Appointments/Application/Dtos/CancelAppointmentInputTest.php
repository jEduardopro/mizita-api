...
dataset('identifiers no cancel input could accept', [
    'empty' => '',
    'whitespace only' => '   ',
    'an integer key' => '42',
    'a word' => 'not-a-uuid',
...
it('accepts a well formed appointment and account', function () {
    $input = new CancelAppointmentInput(AppointmentFixtures::APPOINTMENT_ID, AppointmentFixtures::ACCOUNT_ID);
...
it('refuses an appointment identifier that is no uuid', function (string $appointmentId) {
    expect(fn () => (new CancelAppointmentInput($appointmentId, AppointmentFixtures::ACCOUNT_ID))->validate())
        ->toThrow(AppointmentNotFound::class);
})->with('identifiers no cancel input could accept');

it('refuses an account that is no uuid', function (string $accountId) {
    expect(fn () => (new CancelAppointmentInput(AppointmentFixtures::APPOINTMENT_ID, $accountId))->validate())
...
it('refuses a malformed account as forbidden, with a failure the transport can classify', function () {
    ...
    try {
        (new CancelAppointmentInput(AppointmentFixtures::APPOINTMENT_ID, 'nobody'))->validate();
...
it('judges the appointment before the account', function () {
    expect(fn () => (new CancelAppointmentInput('not-a-uuid', 'nobody'))->validate())
        ->toThrow(AppointmentNotFound::class);
});
