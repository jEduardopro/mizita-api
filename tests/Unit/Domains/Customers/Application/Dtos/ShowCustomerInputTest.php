...
it('carries the customer the route named', function () {
    expect((new ShowCustomerInput('01930000-0000-7000-8000-0000000000c1'))->customerId)
        ->toBe('01930000-0000-7000-8000-0000000000c1');
});

it('accepts a uuid however it is cased', function (string $id) {
    expect(fn () => (new ShowCustomerInput($id))->validate())->not->toThrow(Throwable::class);
})->with([
...
it('refuses an id no customer could ever have, before any repository is asked', function (string $id) {
    expect(fn () => (new ShowCustomerInput($id))->validate())->toThrow(CustomerNotFound::class);
})->with([
    'empty' => '',
    'whitespace only' => '   ',
    'a row number' => '42',
...
it('refuses with a failure the responder renders as a missing page rather than a bad request', function () {
    ...
    try {
        (new ShowCustomerInput('not-a-uuid'))->validate();
