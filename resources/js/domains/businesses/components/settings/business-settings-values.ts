...
export function policyNoticeFrom(settings: BusinessSettings | null): string | null {
    if (settings === null || ! settings.booking_policy.display_on_booking_page) {
        return null;
    }

    return settings.booking_policy.policy_message ?? '';
}
